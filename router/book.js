...
router.get('/front/book/list', async (req, res) => {
  try {
    ...
    let sql = 'SELECT id, book_name AS name, author,author_into, category, price, cover, `desc`, mulu, stock, status, avg_score, comment_count FROM book WHERE status = 1';
    const sqlParams = [];
...
router.get('/front/book/detail', async (req, res) => {
  try {
    ...
    const sql = 'SELECT id, book_name AS name, author,author_into, category, price, cover, `desc`, mulu, stock, status, avg_score, comment_count  FROM book WHERE id = ?';
