...
router.get('/front/book/list', async (req, res) => {
  try {
    ...
    let sql = 'SELECT id, book_name AS name, author, category, price, cover, `desc`, mulu, stock, status FROM book WHERE status = 1';
    const sqlParams = [];
...
router.get('/front/book/detail', async (req, res) => {
  try {
    ...
    const sql = 'SELECT id, book_name AS name, author, category, price, cover, `desc`, mulu, stock, status FROM book WHERE id = ?';
