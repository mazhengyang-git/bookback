...
export const editCommentApi = (data: any) => {
  return request.post('/api/comment/edit1', data)
}
// 2. 发表追评
export const addReplyApi = (data: any) => {
  return request.post('/api/comment/reply/add1', data)
}
// 3. 获取追评列表
export const getReplyListApi = (commentId: number, source: string) => {
  return request.get('/api/comment/reply/list1', { params: { commentId, source } })
}

export const deleteReplyApi = (params: { replyId: number; commentId: number; source: string; }) => {
  return request({
    url: '/api/comment/deleteReply',
    method: 'post',
    data: params
  })
}